...
conflict resolutions and its `allowInTypeGuards` behavior. Prefer Valibot parsing
...
Allow namespace imports used by Valibot and Effect. Keep React-specific rules
out of Solid code. Do not enable Effect-specific rules across application code;
Effect remains an infrastructure dependency. Lint `apps/web/src/ui` along with
...
Use `@t3-oss/env-core` with existing Valibot schemas. Separate public build-time
...
Compare every affirmative dependency choice in STACK.md against package
...
- TanStack Solid Form is absent from the manifest although STACK.md says it is
  ...
  adapter compatibility before installing and using it.
