...
Use **Drizzle ORM v1 RC** with D1 through the `drizzle-orm/d1` driver, wrapped
in the `Drizzle` service in `packages/db` (a promise-based handle exposed as
`run`/`batch` Effect methods). The `Database` service is the single
raw-handle seam: `Drizzle` and the Better Auth `drizzleAdapter` both derive
from it, so one isolate holds one D1 handle. Migrations are generated by
`drizzle-kit generate` into `packages/db/migrations` and applied by Alchemy's
`migrations` option (`drizzle_migrations` bookkeeping table); `drizzle-kit
migrate` is not used. Stay on one query API — Effect-facing query code goes
through `Drizzle` and nothing else holds a drizzle handle.
