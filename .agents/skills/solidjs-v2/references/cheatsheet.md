<!-- Verbatim copy of packages/solid/CHEATSHEET.md from github.com/solidjs/solid (commit 5eb3250a, solid-js@2.0.0-rc.5). Do not edit; refresh from upstream when the prerelease advances. -->
...
setStore((s) => ({ ...s, list: s.list.filter((x) => x !== "x") }));
setStore((s) => ({ ...s, list: [] }));
...
**Value props use reactive values by default.** Two rules share one underlying
model and prevent the most common generated-code bug in Solid. A component may
still declare an accessor prop when its contract intentionally requires one.
...
Context is for state scoped to a subtree of the component tree. Browser-only
state shared by the whole app may use a module-scope signal or store. During
SSR and server execution, create request-specific and user-specific state per
request or inside a component owner. That's why the default-less form requires
a Provider.
...
- **Value props use values by default** — at the call site call accessors
  (`<X v={count()} />`). Pass an accessor only when the component's declared
