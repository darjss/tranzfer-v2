...
- **Returning a value** from the setter performs a shallow replacement: arrays
  ...
  it when mutation is awkward (filter/remove). Replace the array property on
  this object store with
  `setStore(s => ({ ...s, list: s.list.filter(x => x !== "x") }))` — note: no keyed
...
Returns a draft-setter function. Call it _inside_ the setter, targeting the
part of the draft to reconcile (identity preserved for unchanged entries):

```ts
setStore((s) => {
  reconcile(serverTodos)(s.todos);
}); // omitted key = "id"
...
const [view] = createOptimisticStore<{ items: readonly Item[] }>(() => ({ items }), { items: [] });
...
setState((s) => {
  s.cache.set("a", 1);
}); // internal mutation: inert
setState((s) => {
  s.cache = new Map(s.cache).set("a", 1);
}); // reactive slot swap
```
