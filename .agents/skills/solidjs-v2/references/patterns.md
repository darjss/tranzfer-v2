...
function createCachedQuery<K, T>(options: {
  ...
  const state = createMemo<QueryState<K, T>>(async function* (prev) {
    ...
    yield* streamQuery({
      key,
      previous: isRefresh ? prev : undefined,
      signal: abort.signal,
      ...options,
    });
  });
  ...
  const pending = createMemo(
    () => {
      ...
      try {
        return isPending(state) || state().refresh.status === "refreshing";
      } catch {
        return false;
      } // tolerate not-ready on first read
    },
    { lazy: true },
  );
  ...
  return {
    ...
    get error() {
      // surface a failed refresh — pending stays false on failure
      try {
        const r = state().refresh;
        return r.status === "failed" ? r.error : undefined;
      } catch {
        return undefined;
      }
    },
...
async function* streamQuery({ key, previous, signal, fetcher, cache }) {
  ...
  if (previous) {
    // refresh path: keep showing prev, swap when network lands
    ...
    try {
      ...
      yield { ...previous, key, refresh: { status: "failed", error } };
    }
    return;
  }
  const cached = await cache.get(key); // initial path: cache fast, network when ready
  if (cached !== undefined)
    yield { key, source: "cache", value: cached, refresh: { status: "refreshing" } };
  yield { key, source: "network", value: await network, refresh: { status: "ok" } };
}
```
...
- **`onCleanup` + `AbortController` in compute** — superseded runs cancel their
  ...
  _Cancellation & cleanup_ for why `try/finally` can't replace it).
...
- **Facade with getters** — `query.data` reads stay reactive (property access
  ...
  the _binding_ level (`const q = createCachedQuery(...)`; never destructure
...
→ _`isPending` — question-scoped pending_, _Optimistic writes are
verdict-inert_): a bare `refresh(todos)` is a **quiet, same-question re-ask**
...
drive _this_ mutation's "Saving…" spinner off `isPending(() => todos.length)`
either way — co-write a flag into the row (`{ ...todo, pending: true }`) or
use a separate `createOptimistic(false)`. If the reconcile _should_ read as
pending (e.g. you want a subtle "updating…" indicator while it settles),
declare it: `affects(todos); refresh(todos)` (`async-and-actions.md` → *`affects(target, key?)`).
...
generator parked on an _external_ `await` (the next socket message) won't unwind from
...
is an up-front `onCleanup` that **actively cancels** the source — which frees it _and_
unblocks the parked `await` so the generator can unwind. (Full treatment, plus the
plain-async tier: `async-and-actions.md` → _Cancellation & cleanup_.)
...
function subscribeToAsyncIterable<T>(subscribe: (emit: (value: T) => void) => Unsubscribe): {
...
## Browser-only app-wide state

Context scopes state to a subtree. Browser-only state shared by the whole app
may use a module-scope store:
...
Keep request-specific and user-specific state out of module scope during SSR
and server execution. Create that state per request or inside a component owner.
