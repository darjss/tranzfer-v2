...
already discards the superseded _value_, so the reactive result is correct either
...
`action()`, where the generator's `return` _is_ the action's result:
...
but a generator parked on an _external_ `await` (the next socket message, an emitter
...
unwind. Full pattern in `references/patterns.md` → _Streaming a socket_.
...
- `on` prop: re-show fallback when the expression changes _while_ async is
...
hasn't _revealed_ yet, or it carries a live `affects()` mark (below).
...
- It can live outside a boundary only when it reads upstream state that can
  ...
  `isPending(id)` stays `false` while a _downstream_ subtree loads.
...
change the _tracked input_ (id, query key) — normally reveal their fresh value
...
but do not write correctness logic that depends on it _never_ pulsing in rc.5.
...
mark can turn pending _on_ for data the graph can't otherwise see changing;
nothing can turn pending _off_ while a real change is in flight. `affects`
...
- `affects(store)` — every record **reachable from `store` at declaration
  ...
  _added_ to the store after the call are not covered.
...
_expected_ value; `affects` pends data you know is changing but can't show a
...
never a verdict read off `isPending`. See _Optimistic primitives_ below for
...
const acknowledged = await until(() => todos.find((todo) => todo.clientId === clientId), {
  timeout: 5_000,
  signal,
});
```
...
const addTodo = action(async function* (todo: Todo) {
  ...
  yield until(() => todos.some((row) => row.id === todo.id), { timeout: 5_000 });
});
```
...
between an internal `await` and the _next_ `yield` escape the action's
