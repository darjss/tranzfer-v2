...
primitive config (theme, locale). Browser-only app-wide state may use a
module-scope signal or store. In SSR and server code, keep request-specific and
user-specific state inside the request or component owner so one request cannot
read another request's state.
